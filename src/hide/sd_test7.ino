...
//   pinMode(SS, Output);
  ...
  if (!SD.exists(FILENAME)) {
    ...
    myFile.println("hello cruel world");
    myFile.close();
  }

  // recheck if file is created or not
  if (SD.exists(FILENAME))
    Serial.println(F("mytest.txt exists on SD Card."));
  else
    Serial.println(F("mytest.txt doesn't exist on SD Card."));
}
