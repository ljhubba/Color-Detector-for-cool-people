...
#define BUTTON_PIN 4  // Pin for button input to trigger actions in the program.
#define YELLOW_LED 9  // Pin for Yellow LED output to indicate yellow color detection.
#define GREEN_LED 7  // Pin for Green LED output to indicate green color detection.
#define RED_LED 6 
#define BLUE_LED 8 
#define PHOTOTRANSISTOR_PIN 23
#define TOGGLE_SWITCH_PIN 3 
#define POTENTIOMETER_PIN 24 

LiquidCrystal lcd(18,16,17,19,20,21); //change to whatever pins we use
...
void getDataFromCard()
{
    ...
    Serial.println(groupNumber);
		myFile = SD_MMC.open(String(groupNumber) + ".txt", FILE_READ);
		while(1)
		{
...
void setup()
{
  ...
  for (int i = 0; i < 10; i++){
   Serial.print("yo dabba dabba doo");
   Serial.print(i);
   delay(250);
  }
  Serial.print("STARTUP TEST COMPLETED");
  lcd.begin(16, 2);
...
void loop()
{
  ...
  if (!digitalRead(BUTTON_PIN) && !colorDetector.currentlyReading) {
    ...
    if (colorDetector.trainingModeActive) {
      ...
      File myFile = SD_MMC.open(String(colorDetector.selectedGroupId) + ".txt", FILE_WRITE);
